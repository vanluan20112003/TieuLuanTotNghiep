...
<button class="toolbar-btn" onclick="openAddNewCategoryModal()"> 
    <i class="fas fa-plus-circle"></i> Thêm danh mục
</button>

<button class="toolbar-btn" onclick="openCategoryStaticModal()"> 
...
<div id="editCategoryModal" class="edit-category-modal hidden">
    <div class="modal-content">
        <h2>Chỉnh sửa danh mục</h2>
        <form id="edit-category-form">
            <label for="edit-category-id">ID danh mục:</label>
            <input type="text" id="edit-category-id" disabled>

            <label for="edit-category-name">Tên danh mục:</label>
            <input type="text" id="edit-category-name" required>

            <label for="edit-category-product-count">Số lượng sản phẩm:</label>
            <input type="text" id="edit-category-product-count" disabled>

            <label for="edit-category-created-at">Ngày thêm:</label>
            <input type="text" id="edit-category-created-at" disabled>

            <label for="edit-category-image">Ảnh:</label>
            <input type="file" id="edit-category-image">
            <img id="edit-category-image-preview" style="width: 100px; height: 100px; object-fit: cover; margin-top: 10px;">

            <div class="modal-actions">
                <button type="button" id="save-category-btn">Lưu</button>
                <button type="button" id="cancel-category-btn">Hủy</button>
            </div>
        </form>
    </div>
</div>
...
<!-- Modal View Category -->
<div id="viewCategoryModal" class="view-category-modal hidden">
    <div class="modal-content">
        <h2 id="view-category-label">Danh sách sản phẩm của danh mục</h2>
        <table>
            <thead>
                <tr>
                    <th>STT</th>
            ...
            <tbody id="category-product-list">
                <!-- Dữ liệu sẽ được tải động qua AJAX -->
            </tbody>
        </table>
        <button id="close-view-category-modal" class="close-modal-btn">Đóng</button>
    </div>
</div>
<div id="addCategoryModal" class="add-category-modal hidden">
    <div class="modal-content">
        ...
        <form id="add-category-form">
            ...
            <label for="add-category-image">Ảnh:</label>
            <input type="file" id="add-category-image" required>
            <img id="add-category-image-preview" style="width: 100px; height: 100px; object-fit: cover; margin-top: 10px;">

            <div class="modal-actions">
                <button type="button" id="add-category-btn">Lưu</button>
                <button type="button" id="cancel-category-btn" onclick="closeAddCategoryModal()">Hủy</button>
            </div>
        </form>
    </div>
</div>
<!-- Modal Category Static -->
<div id="categoryStaticModal" class="category-static-modal">
    <div class="category-static-modal-content">
        <span class="category-static-close-btn" onclick="closeCategoryStaticModal()">&times;</span>
        ...
        <button onclick="fetchCategoryStatistics()">Lọc</button>

        <!-- Hiển thị thông tin thống kê -->
        <div style="max-height: 400px; overflow-y: auto; margin-top: 20px;">
            <table id="categoryStaticTable" class="category-static-table" style="width: 100%;">
                <thead>
                    <tr>
                        <th>Tên Danh Mục</th>
                        <th>Số Sản Phẩm</th>
                ...
                <tbody>
                    <!-- Dữ liệu thống kê sẽ được chèn ở đây -->
                </tbody>
            </table>
        </div>

        <!-- Các button chuyển đổi biểu đồ -->
        <div class="chart-controls" style="margin-top: 20px;">
       ...
       <!-- Biểu đồ thống kê -->
<div>
    <canvas id="categoryStaticChart" width="300" height="100"></canvas> <!-- Giảm chiều rộng và chiều cao -->
</div>
...
function fetchCategories() {
    fetch('http://localhost/web_ban_banh_kem/public/admin/categories') // Thay URL bằng API endpoint của bạn
        .then(response => response.json())
        .then(data => {
            const categoryTable = document.getElementById('CateogoryTable');
            ...
            data.categories.forEach(category => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="image-cell">
                        <img src="http://localhost/web_ban_banh_kem/public/images/${category.image}" alt="${category.name}" style="width: 50px; height: 50px; object-fit: cover;">
                    </td>
                    <td>${category.id}</td>
                    <td>${category.name}</td>
                    ...
                    <td>
                        ...
                        <button class="view-btn" data-id="${category.id}">Xem</button>
                    </td>
                `;
...
function editCategory(id) {
    // Gọi API lấy thông tin chi tiết danh mục
    fetch(`http://localhost/web_ban_banh_kem/public/admin/categories/${id}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Hiển thị modal
                ...
                document.getElementById('edit-category-product-count').value = data.category.product_count;
                document.getElementById('edit-category-created-at').value = new Date(data.category.created_at).toLocaleDateString();
                document.getElementById('edit-category-image-preview').src = 'http://localhost/web_ban_banh_kem/public/images/' + data.category.image;


                // Gắn sự kiện lưu danh mục
                const saveButton = document.getElementById('save-category-btn');
                saveButton.onclick = function () {
                    saveCategoryChanges(id);
                };
            } else {
...
function saveCategoryChanges(id) {
    ...
    fetch(`http://localhost/web_ban_banh_kem/public/admin/categories/${id}`, {
        method: 'POST',
        headers: {
        'X-CSRF-TOKEN': token, // Thêm token CSRF vào header
    },
        body: formData,
    })
        .then(response => response.json())
        ...
        .catch(error => console.error('Lỗi khi lưu danh mục:', error));
}

function deleteCategory(id) {
    if (confirm(`Bạn có chắc muốn xóa danh mục có ID: ${id}?`)) {
        fetch(`http://localhost/web_ban_banh_kem/public/admin/categories/${id}`, {
            method: 'DELETE',
            headers: {
...
// Sự kiện nút "Hủy"
document.getElementById('cancel-category-btn').addEventListener('click', closeEditCategoryModal);

// Sự kiện bấm ra ngoài modal
...
// Thay đổi ảnh khi upload file mới
document.getElementById('edit-category-image').addEventListener('change', function (event) {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            // Gán ảnh vào phần preview
            document.getElementById('edit-category-image-preview').src = e.target.result;
        };

        reader.readAsDataURL(file); // Đọc file dưới dạng URL
    } else {
        // Nếu không có file, có thể đặt lại ảnh preview
        document.getElementById('edit-category-image-preview').src = '';
    }
});
// Thêm sự kiện cho nút "Check All"
document.querySelector('.checkbox-cell input[type="checkbox"]').addEventListener('change', function () {
    const isChecked = this.checked; // Lấy trạng thái của checkbox "Check All"
    const childCheckboxes = document.querySelectorAll('#CateogoryTable input[type="checkbox"]'); // Các checkbox con

    // Duyệt qua tất cả các checkbox con và thay đổi trạng thái theo "Check All"
    childCheckboxes.forEach(checkbox => {
        checkbox.checked = isChecked;
    });
});
// Hàm mở modal và hiển thị danh sách sản phẩm theo danh mục
function viewCategory(categoryId) {
    // Fetch dữ liệu sản phẩm theo ID danh mục
    fetch(`http://localhost/web_ban_banh_kem/public/admin/categories/${categoryId}/products`)
        .then(response => response.json())
        .then(data => {
            const modal = document.getElementById('viewCategoryModal');
            const modalLabel = document.getElementById('view-category-label');
            const productList = document.getElementById('category-product-list');

            // Cập nhật label của modal
            modalLabel.textContent = `Danh sách sản phẩm của danh mục: ${data.category.name} (${data.products.length} sản phẩm)`;
            ...
            data.products.forEach((product, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${product.id}</td>
                    <td>
                        <img src="http://localhost/web_ban_banh_kem/public/images/${product.image}" alt="${product.name}" style="width: 50px; height: 50px; object-fit: cover;">
                    </td>
                    <td>${product.name}</td>
                `;
...
// Đóng modal khi bấm "Đóng" hoặc bấm ra ngoài
document.getElementById('close-view-category-modal').addEventListener('click', function () {
...
// Gắn sự kiện click vào các nút "View"
document.querySelectorAll('.view-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        const id = this.getAttribute('data-id');
        viewCategory(id);
...
// Sự kiện "In tất cả" (Export tất cả)
document.getElementById('exportAllBtn').addEventListener('click', function () {
    fetch('http://localhost/web_ban_banh_kem/public/admin/categories')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                exportToExcel(data.categories); // Xuất tất cả dữ liệu
            }
        });
});

// Sự kiện "In đã chọn" (Export các mục được chọn)
document.getElementById('exportSelectedBtn').addEventListener('click', function () {
    ...
    document.querySelectorAll('.category-checkbox:checked').forEach(checkbox => {
        ...
        selectedCategories.push({
            ...
            product_count: row.cells[4].textContent,
            created_at: row.cells[5].textContent,
        });
    });

    if (selectedCategories.length > 0) {
        exportToExcel(selectedCategories); // Xuất các mục được chọn
    } else {
...
function exportToExcel(categories) {
    ...
    const ws = XLSX.utils.json_to_sheet(categories); // Chuyển đổi dữ liệu JSON thành worksheet
...
// Sự kiện chọn ảnh
document.getElementById('add-category-image').addEventListener('change', function() {
    const reader = new FileReader();
    reader.onload = function(e) {
        document.getElementById('add-category-image-preview').src = e.target.result;
    }
...
document.getElementById('add-category-btn').addEventListener('click', function() {
    ...
    const image = document.getElementById('add-category-image').files[0];
    ...
    if (name && image) {
        ...
        fetch('http://localhost/web_ban_banh_kem/public/admin/categories/add', {
            method: 'POST',
            headers: {
        'X-CSRF-TOKEN': token, // Thêm token CSRF vào header
    },
        ...
        .then(data => {
            if (data.success) {
                ...
                closeAddCategoryModal(); // Đóng modal sau khi lưu
        ...
        alert('Vui lòng nhập tên và chọn ảnh cho danh mục.');
    }
});
// Hàm mở Modal Category Static
// Biến toàn cục để lưu loại biểu đồ hiện tại
let chartType = 'bar';
let chart;  // Biến để lưu đối tượng biểu đồ

// Hàm mở Modal Category Static
function openCategoryStaticModal() {
    document.getElementById('categoryStaticModal').style.display = 'block';
}

// Hàm đóng Modal Category Static
function closeCategoryStaticModal() {
    document.getElementById('categoryStaticModal').style.display = 'none';
}

// Hàm gọi API và hiển thị thống kê
function fetchCategoryStatistics() {
    const startDate = document.getElementById('start_date').value;
    const endDate = document.getElementById('end_date').value;

    // Xây dựng URL để gọi API với tham số start_date và end_date
    let url = `http://localhost/web_ban_banh_kem/public/admin/categories/statistics?start_date=${startDate}&end_date=${endDate}`;
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.statistics && data.statistics.length > 0) {
                // Lọc và hiển thị dữ liệu thống kê trong bảng
                const tableBody = document.getElementById('categoryStaticTable').getElementsByTagName('tbody')[0];
                ...
                data.statistics.forEach(stat => {
                    ...
                    // Lưu tên danh mục và doanh thu cho biểu đồ
                    categoryNames.push(stat.category_name);
                    totalRevenues.push(stat.total_revenue);
                });

                // Vẽ biểu đồ doanh thu
                drawCategoryRevenueChart(categoryNames, totalRevenues);
            } else {
...
// Hàm vẽ biểu đồ doanh thu theo danh mục
// Hàm vẽ biểu đồ doanh thu theo danh mục
function drawCategoryRevenueChart(labels, data) {
    const ctx = document.getElementById('categoryStaticChart').getContext('2d');
        ...
        options: {
            ...
            plugins: {
                legend: {
                    ...
                    position: 'top',
                    labels: {
                        boxWidth: 10 // Giảm kích thước chú thích
                    }
                }
            },
            scales: {
                y: {
                    beginAtZero: true,
                    ticks: {
                        maxTicksLimit: 5 // Giới hạn số lượng điểm trên trục Y
                    }
                },
                x: {
                    ticks: {
                        maxRotation: 45, // Xoay nhãn trục X để tiết kiệm không gian
                        minRotation: 45
                    }
                }
            }
...
function changeChartType(type) {
    chartType = type; // Lưu loại biểu đồ đã chọn
    fetchCategoryStatistics(); // Lấy lại thống kê và vẽ lại biểu đồ
}
